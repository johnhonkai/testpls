...
function selectTabMobile(event) {
  ...
  onMount(async () => {
    ...
    fovlikes = await (await import("$lib/firebaseLikes")).getLikesFromFirestore(charName); // Fetch current likes
  });
  ...
  async function increaseLike() {
    ...
    fovlikes = await likeWithVoterId(charName, voterId); // Like with voter ID tracking
