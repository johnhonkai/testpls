...
function selectTabMobile(event) {
  ...
  let fovlikes = 0;
  let fovhasLiked = false;

  // Check local storage to see if the user has already liked
  onMount(() => {
    fovhasLiked = JSON.parse(localStorage.getItem('fovhasLiked')) || false;
      // Load the initial likes count from the server or storage
      fovlikes = parseInt(localStorage.getItem('fovlikes')) || 0;
  });
  ...
  function increaseLike() {
      ...
      if (!fovhasLiked) {
        ...
        fovhasLiked = true;
          ...
          localStorage.setItem('fovlikes', fovlikes);
          localStorage.setItem('fovhasLiked', JSON.stringify(fovhasLiked));
      } else {
...
<div class="relative  w-auto h-48 sm:h-60 md:h-72 flex justify-center ">
  ...
  <img src="/images/valkfull/fov.webp" alt="FoV" class="h-full w-auto object-cover md:object-contain  " style ="view-transition-name: valkyrie-image-5;"/> 
