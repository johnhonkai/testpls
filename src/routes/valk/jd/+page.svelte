...
function selectTabMobile(event) {
  ...
  let jdlikes = 0;
  let jdhasLiked = false;

  // Check local storage to see if the user has already liked
  onMount(() => {
    jdhasLiked = JSON.parse(localStorage.getItem('jdhasLiked')) || false;
      // Load the initial likes count from the server or storage
      jdlikes = parseInt(localStorage.getItem('jdlikes')) || 0;
  });
  ...
  function increaseLike() {
      ...
      if (!jdhasLiked) {
        ...
        jdhasLiked = true;
          ...
          localStorage.setItem('jdlikes', jdlikes);
          localStorage.setItem('jdhasLiked', JSON.stringify(jdhasLiked));
      } else {
