...
function selectTabMobile(event) {
  ...
  let lanternlikes = 0; // Default likes
  ...
  onMount(async () => {
    ...
    lanternlikes = await (await import("$lib/firebaseLikes")).getLikesFromFirestore(charName); // Fetch current likes
  });
  ...
  async function increaseLike() {
    ...
    lanternlikes = await likeWithVoterId(charName, voterId); // Like with voter ID tracking
