...
function selectTabMobile(event) {
  ...
  let durandallikes = 0; // Default likes
  ...
  onMount(async () => {
    ...
    durandallikes = await (await import("$lib/firebaseLikes")).getLikesFromFirestore(charName); // Fetch current likes
  });
  ...
  async function increaseLike() {
    ...
    durandallikes = await likeWithVoterId(charName, voterId); // Like with voter ID tracking
