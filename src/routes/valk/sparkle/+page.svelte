...
function selectTabMobile(event) {
  ...
  let sparklelikes = 0; // Default likes
  ...
  onMount(async () => {
    ...
    sparklelikes = await (await import("$lib/firebaseLikes")).getLikesFromFirestore(charName); // Fetch current likes
  });
  ...
  async function increaseLike() {
    ...
    sparklelikes = await likeWithVoterId(charName, voterId); // Like with voter ID tracking
