...
function selectTabMobile(event) {
    ...
    onMount(() => {
        hasLiked = JSON.parse(localStorage.getItem('hasLiked')) || false;
    ...
    function increaseLike() {
        ...
        if (!hasLiked) {
            ...
            localStorage.setItem('hasLiked', JSON.stringify(hasLiked));
        } else {
  ...
  <div class="relative  w-auto h-48 sm:h-60 md:h-72 flex justify-center ">
    ...
    <div class="absolute bottom-0 left-0 like-container flex items-center gap-2 mt-4">
      <button on:click={increaseLike} class="bg-gray-800 text-white px-4 py-2 rounded hover:bg-blue-700 transition-all">
        🩷 <span class="text-white font-semibold">{likes}</span>
      </button>
