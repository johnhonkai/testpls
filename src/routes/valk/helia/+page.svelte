...
function selectTabMobile(event) {
  ...
  let helialikes = 0; // Default likes
  ...
  onMount(async () => {
    ...
    helialikes = await (await import("$lib/firebaseLikes")).getLikesFromFirestore(charName); // Fetch current likes
  });
  ...
  async function increaseLike() {
    ...
    helialikes = await likeWithVoterId(charName, voterId); // Like with voter ID tracking
