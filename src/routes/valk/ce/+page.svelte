...
function selectTabMobile(event) {
  ...
  onMount(async () => {
    ...
    celikes = await (await import("$lib/firebaseLikes")).getLikesFromFirestore(charName); // Fetch current likes
  });
  ...
  async function increaseLike() {
    ...
    celikes = await likeWithVoterId(charName, voterId); // Like with voter ID tracking
...
<div class="flex flex-col items-center text-center justify-start">
  ...
  <div>
  <h1 class="text-xl md:text-2xl text-slate-100 font-bold text-center leading-4 mb-2 sm:mb-0">Cosmic Expression  </h1>
</div>
  <!-- Character Name and Release Date -->
  <p class="text-base md:text-md custom-font mb-2 text-center tracking-wider md:block hidden text-slate-100">Griseo | Release Date: v7.1 (7 Dec 2023)</p>
    ...
    <div class="flex flex-col mt-4 items-center">
      ...
      <div class="flex flex-col  w-[260px] md:w-[300px] flex-wrap justify-center outline outline-blue-500 outline-1 bg-blue-950/75 rounded-lg p-2 backdrop-blur-sm">
        ...
        <div class="flex flex-row gap-2 flex-wrap justify-center">
          <img src="/images/artype/p1bleed.png" alt="Support 2" class="w-auto h-8 md:h-10" />
        </div>
      </div>
    </div>
